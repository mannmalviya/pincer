 ...
 *     newBody?: string,           // present iff the model proposed an edit
 *   }
 *
 * Response (4xx/5xx):
 *   { error: string }
...
function buildSystemPrompt(req: ChatRequest): string {
  ...
  return [
    ...
    "",
    "How you reply:",
    'You MUST respond with a single JSON object: {"reply": "...", "new_body": "..."}',
    '- "reply" (required): your conversational response to the user.',
    '- "new_body" (optional): if you are proposing an edit, the FULL new text of the post body. Omit this field if you are just chatting (asking a clarifying question, giving feedback without changes, etc.).',
    "When proposing an edit, return the complete new body, NOT a diff or fragment. The dashboard computes the diff itself.",
...
// Parse the model's response into {reply, new_body}. Tolerant: if the
// model returned plain text (ignored JSON mode), we treat the whole
// thing as the chat reply with no edit. Best-effort `JSON.parse` first,
// then a regex extraction of the outermost `{...}` block in case the
// model wrapped its JSON in stray prose.
  ...
  const tryJson = (text: string) => {
    try {
      ...
      if (typeof parsed.reply === "string") {
        const out: { reply: string; newBody?: string } = {
          reply: parsed.reply,
        };
